...
titanic_glm_ai <- predict_aspects(x = explain_titanic_glm, 
...
To use predict_aspects() with lasso, we have to provide `n_var` parameter, which declares how many aspects importance values we would like to get in predict_aspects() results.

For this example, we use `titanic_imputed` dataset again and random forest model. With the help of lasso technique, we would like to check the importance of variables' aspects, while controlling that one of them should be equal to 0. Therefore we call predict_aspects() with `n_var` parameter set to 3. 
...
Afterwards, we call predict_aspects() function with parameter `show_cor = TRUE`, to check how features are grouped into aspects, what is minimal value of pairwise correlation in each group and to check whether any pair of features is negatively correlated (`neg`) or not (`pos`). 
...
apartments_ai <- predict_aspects(x = explain_apartments, 
...
apartments_tri <- calculate_triplot(explain_apartments,
                                    new_observation = new_observation_apartments,
